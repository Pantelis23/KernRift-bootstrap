...
pub(super) fn normalize_policy(
    ...
    for symbol in &mut policy.kernel.allow_raw_mmio_in_irq_symbols {
        *symbol = symbol.trim().to_string();
        if symbol.is_empty() {
            return Err(format!(
                "invalid policy '{}': allow_raw_mmio_in_irq_symbols entries must be non-empty strings",
                source_name
            ));
        }
    }
    policy.kernel.allow_raw_mmio_in_irq_symbols.sort();
    policy.kernel.allow_raw_mmio_in_irq_symbols.dedup();
