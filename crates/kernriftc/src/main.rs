...
struct ArtifactInspectionReport {
    artifact_kind: &'static str,
    file_size: usize,
    #[serde(skip_serializing_if = "Option::is_none")]
    machine: Option<&'static str>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pointer_bits: Option<u16>,
    #[serde(skip_serializing_if = "Option::is_none")]
    endianness: Option<&'static str>,
    symbols: Vec<ArtifactInspectionSymbol>,
    defined_symbols: Vec<String>,
    undefined_symbols: Vec<String>,
    relocations: Vec<ArtifactInspectionRelocation>,
    #[serde(skip_serializing_if = "Option::is_none")]
    asm: Option<ArtifactInspectionAsmSummary>,
    flags: ArtifactInspectionFlags,
}

#[derive(Debug, Clone, Serialize)]
struct ArtifactInspectionSymbol {
    name: String,
    category: &'static str,
...
#[derive(Debug, Clone, Serialize)]
struct ArtifactInspectionRelocation {
    section: String,
    #[serde(rename = "type")]
    reloc_type: String,
    target: String,
...
#[derive(Debug, Clone, Serialize)]
struct ArtifactInspectionAsmSummary {
    globals: Vec<String>,
    labels: Vec<String>,
    direct_call_targets: Vec<String>,
    appears_x86_64_text_subset: bool,
}

#[derive(Debug, Clone, Serialize)]
struct ArtifactInspectionFlags {
    has_entry_symbol: bool,
    has_undefined_symbols: bool,
...
fn inspect_artifact_from_bytes(bytes: &[u8]) -> Result<ArtifactInspectionReport, String> {
    if bytes.len() >= 4 && &bytes[0..4] == b"KRBO" {
        return inspect_krbo_artifact(bytes);
    }
    if bytes.len() >= 4 && &bytes[0..4] == b"\x7fELF" {
        return inspect_elf_artifact(bytes);
    }
    if let Ok(report) = inspect_asm_artifact(bytes) {
        return Ok(report);
    }
    Err("inspect-artifact: unsupported artifact bytes".to_string())
}

fn format_artifact_inspection_report_text(report: &ArtifactInspectionReport) -> String {
...
fn inspect_krbo_artifact(bytes: &[u8]) -> Result<ArtifactInspectionReport, String> {
...
fn inspect_elf_artifact(bytes: &[u8]) -> Result<ArtifactInspectionReport, String> {
    if bytes.len() < 64 {
        return Err(
    ...
    if e_shnum > 0 {
        ...
        if sh_end > bytes.len() {
            return Err("inspect-artifact: failed to parse ELF artifact: section table exceeds artifact size".to_string());
        }
...
fn inspect_asm_artifact(bytes: &[u8]) -> Result<ArtifactInspectionReport, String> {
...
fn parse_nul_terminated_utf8(bytes: &[u8], offset: usize, context: &str) -> Result<String, String> {
    if offset >= bytes.len() {
        return Err(format!(
...
fn read_u16_at(bytes: &[u8], offset: usize, little: bool, context: &str) -> Result<u16, String> {
...
fn read_u32_at(bytes: &[u8], offset: usize, little: bool, context: &str) -> Result<u32, String> {
...
fn read_u64_at(bytes: &[u8], offset: usize, little: bool, context: &str) -> Result<u64, String> {
...
fn is_asm_symbol_name(symbol: &str) -> bool {
