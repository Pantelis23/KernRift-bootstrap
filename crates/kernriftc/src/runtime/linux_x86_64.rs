...
pub static BLOB: RuntimeBlob = RuntimeBlob {
    code: &[
        // === _start (offset 0x00) ===
        // mov rdi, [rsp]                  ; argc
        0x48, 0x8b, 0x3c, 0x24, // lea rsi, [rsp+8]               ; argv
        0x48, 0x8d, 0x74, 0x24, 0x08, // lea rax, [rdi+1]               ; argc+1
        0x48, 0x8d, 0x47, 0x01, // lea rdx, [rsi+rax*8]           ; envp
        0x48, 0x8d, 0x14, 0xc6, // mov [rip+0x1a0], rdx           ; save envp
        0x48, 0x89, 0x15, 0xa0, 0x01, 0x00, 0x00,
        // sub rsp, 8                     ; align stack
        0x48, 0x83, 0xec, 0x08, // call main (rel32 placeholder)  ; FIXUP at offset 0x1d
        0xe8, 0x91, 0x01, 0x00, 0x00, // mov edi, eax                   ; exit code
        0x89, 0xc7,
        // === __kr_exit (offset 0x23) ===
        // mov eax, 231                   ; __NR_exit_group
        0xb8, 0xe7, 0x00, 0x00, 0x00, // syscall
        0x0f, 0x05,
        ...
        0x41, 0xba, 0x22, 0x00, 0x00, 0x00, // mov r8, -1                     ; fd = -1
        0x49, 0xc7, 0xc0, 0xff, 0xff, 0xff, 0xff,
        // xor r9d, r9d                   ; offset = 0
        0x45, 0x31, 0xc9, // mov eax, 9                     ; __NR_mmap
        0xb8, 0x09, 0x00, 0x00, 0x00, // syscall
        0x0f, 0x05, // ret
        0xc3,
        // === __kr_alloc (offset 0x54) ===
        // add rdi, 15                    ; align to 16
        0x48, 0x83, 0xc7, 0x0f, // and rdi, -16
        0x48, 0x83, 0xe7, 0xf0, // mov rax, [rip+0x165]           ; heap_remaining
        0x48, 0x8b, 0x05, 0x65, 0x01, 0x00, 0x00, // cmp rdi, rax
        0x48, 0x39, 0xc7, // jbe .have_space (+0x1e)
        0x76, 0x1e, // push rdi                       ; save aligned size
        0x57, // mov edi, 0x400000              ; 4MB
        0xbf, 0x00, 0x00, 0x40, 0x00, // call __kr_mmap_alloc (rel32=-0x41)
        ...
        0x48, 0xc7, 0x05, 0x43, 0x01, 0x00, 0x00, 0x00, 0x00, 0x40, 0x00,
        ...
        0x48, 0x29, 0x3d, 0x2d, 0x01, 0x00, 0x00, // ret
        0xc3,
        ...
        0x74, 0x2b, // mov rcx, rdi                   ; save name
        0x48, 0x89, 0xf9, // mov r8, rdx                    ; save entry
        ...
        0x31, 0xc0, // ret
        0xc3, // === __kr_exec (offset 0xda) ===
        // push rbx
        0x53, // push r12
        0x41, 0x54, // mov rbx, rdi                   ; save cmd
        0x48, 0x89, 0xfb, // mov eax, 56                    ; __NR_clone
        0xb8, 0x38, 0x00, 0x00, 0x00, // mov edi, 17                    ; SIGCHLD
        0xbf, 0x11, 0x00, 0x00, 0x00, // xor esi, esi
        0x31, 0xf6, // xor edx, edx
        0x31, 0xd2, // xor r10d, r10d
        0x45, 0x31, 0xd2, // xor r8d, r8d
        0x45, 0x31, 0xc0, // syscall
        ...
        0x45, 0x31, 0xd2, // mov eax, 61                    ; __NR_wait4
        0xb8, 0x3d, 0x00, 0x00, 0x00, // syscall
        ...
        0x2f, 0x62, 0x69, 0x6e, 0x2f, 0x73, 0x68, 0x00, // .dash_c (offset 0x1af): "-c\0"
        0x2d, 0x63, 0x00, // === Padding (offset 0x1b2, 6 bytes) ===
