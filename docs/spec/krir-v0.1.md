...
| `kernriftc policy --format json --policy <policy.toml> --contracts <contracts.json>` | `--format json` | `kernrift_policy_violations_v1` | on pass or deny: `stdout` only, empty `stderr`, trailing newline, `schema_version` present; on invalid input: no JSON payload, deterministic `stderr` | `0` pass, `1` deny, `2` invalid input | pass and deny |
| `kernriftc check --format json --policy <policy.toml> <file.kr>` | `--format json` | `kernrift_policy_violations_v1` on policy denial | on policy deny: `stdout` only, empty `stderr`, trailing newline, `schema_version` present; on invalid input: no JSON payload, deterministic `stderr`; other check failures remain command-specific | `1` on policy deny with JSON payload, `2` on invalid input; other check failures remain command-specific | policy deny only today |
